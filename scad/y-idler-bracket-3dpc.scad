...
axel_height = y_motor_height() + pulley_inner_radius - ball_bearing_diameter(Y_idler_bearing) / 2 + 4;
...
module biggerslot() {
			translate([-17, 12, 2])     // screw slot
				rotate([0,0,90])
					slot(r = screw_clearance_radius(base_screw) + .5, l = slot -8, h = 2 * base_thickness + 1, center = true);
...
module base() {
	difference(){
		...
		translate([0,1.5,base_thickness/2])
		   cube(size=[50,43,base_thickness], center=true );
...
intersection(){
	union() {
		...
		// % translate([10,0,0])  cube([20,40,45.72]);
		// % translate([-10,0,axel_height]) rotate([0,90,0]) cylinder(h = 20,r = (33.02/2),center = false);
	...
	translate([0, (length - tab_length) / 2 - dia / 2, -1])
		rounded_rectangle([50, length - tab_length - eta, height + 2], r = 2, center = false);
}