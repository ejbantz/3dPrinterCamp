...
module wade (hotend_mount=0,legacy_mount=false){
	difference (){
		union(){
			// The wade block.
			cube([wade_block_width,wade_block_height+3,wade_block_depth]);    // EJB added some height
	...
	#	translate ([-7.5, 58, 0]) 
       cube (size = [20, 10, 10]);  // EJB trimming top
...
*/
			...
			for (mount=[0:1]){
				translate([-filament_feed_hole_offset+30*((mount<1)?1:-1),
...
//%cube([12,4,5],center=true);
	...
	difference(){
		...
		translate(idler_axis+[2+guide_height,+idler_long_side-idler_long_bottom-guide_length/2,0]){
			cube([7,guide_length+2,1.75 + .5],center=true);
			translate([-7/2,0,0])
			rotate([90,0,0])
		cylinder(h=guide_length+4,r=(1.75 + 1)/2,center=true,$fn=16);
		}
