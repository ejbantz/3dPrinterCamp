...
module z_linear_bearings(motor_end) {
    ...
    union() {
        difference(){
            ...
            //hole for bearings
            translate([0, 0, -1])
                cylinder(h = bearing_height + 2, r = bearing_dia / 2 + relief);
	        ...
	        translate([bearing_dia/2, 0, bearing_height / 2])
				cube([bearing_dia + relief * 2, bearing_dia + relief * 2, bearing_height + 1], center = true);
            ...
            union(){
                ...
                if(motor_end)
                    ...
                    difference() {
						...
						//		teardrop(10/32*inch / 2 , h = 100, center = true);
                                    nut_trap(10/32*inch/2, M4_nut_radius, M4_nut_trap_depth);
           ...
           translate([0, 0, bearing_height / 2 - thickness / 2])
            ...
            difference() {
                translate([-z_bar_offset(), 0, - thickness / 2])
                    nut_trap((7/16*inch) / 2, Z_nut_radius, nut_shelf + thickness / 2 + nut_trap_depth(Z_nut), supported = integral_support);
                ...
                union() {
                    ...
                    translate([back - mbracket_width / 2 + eta, mbracket_centre, mbracket_height / 2 - thickness / 2]) {
                        difference() {
                            ...
                            translate([0, 50, -mbracket_thickness])
                                 cube([mbracket_width - 2 * mbracket_thickness,                      // inside
                                       mbracket_depth - 2 * mbracket_thickness +100, mbracket_height], center = true);
                    ...
                    for(x = NEMA_holes(X_motor))                                                        // motor screw holes
                        for(z = NEMA_holes(X_motor))
                            rotate([0, motor_angle, 0])
                                translate([x, 0, z])
                                    rotate([90,  -motor_angle, 0]) {
                                     #   translate([-screw_offset, 0, 28.5])
                                            teardrop_plus(r = .25, h = 100);            // front holes
...
module x_end_assembly(motor_end) {
    ...
    // RP bits
    //
    color("red") render() x_end_bracket(motor_end);
   ...
   translate([(front + back) / 2, side * bar_y, thickness / 2])
    ...
    if(motor_end) {
        ...
        translate([x_motor_offset(), x_end_ribbon_clamp_y(), x_end_ribbon_clamp_z()]) {
            rotate([90, 0, 0]) {
                 render() x_motor_ribbon_bracket_stl(false);
...
//x_end_bracket(false, true);

/*
difference() {
x_end_bracket(false, true);
            translate([0,0,7]) tube(h = zipslot_width, ir = bearing_dia / 2 + bwall,
                                        or = bearing_dia / 2 + bwall + zipslot_tickness, fn=64); // 
            translate([0,0,43]) tube(h = zipslot_width, ir = bearing_dia / 2 + bwall,
                                        or = bearing_dia / 2 + bwall + zipslot_tickness, fn=64); // 
