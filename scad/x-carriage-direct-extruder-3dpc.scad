...
spacing_between_hotends = 30;

extruder_width = 26;
function nozzle_x_offset() = 16 - 16;                // offset from centre of the extruder

second_extruder_offset = -53;
...
module base_shape() {
   ...
   //         translate([bearing_holder_length(X_bearings) / 2 + bearing_gap, width / 2 - corner_radius + 35])
   //             circle(r = corner_radius, center = true);

  //          translate([-bearing_holder_length(X_bearings) / 2 - bearing_gap, width / 2 - corner_radius + 35])
  //              circle(r = corner_radius, center = true);

            translate([-length / 2 + corner_radius, extruder_width / 2 + 20])
                circle(r = corner_radius, center = true);
...
translate([0,- wall,0])
minkowski() {
square([length - 4 * wall - 5, width - 4 * wall ], center = true);
circle(r = wall * 2, center = true);
}
...
//        minkowski() {
//           difference() {
//                square([length + 1, width + 1 + 65  ], center = true);
//                translate([10,0])
        ...
        difference(){
                        ...
                        union() {
                            ...
                            difference() {
                                ...
                                translate([0, 0, top_thickness])
                                   ...
                                   #           rounded_square(hole - 0 * wall + 8, hole_width  + 2 * wall, corner_radius + wall);
            ...
            translate([-base_offset, 0, 0]) {
			    ...
			    translate([0, - hole_offset])
                    rounded_rectangle([hole + 2, hole_width , 2 * rim_thickness], corner_radius);
...
module set_of_hotend_nut_holders(distance){
	for(side = [-1,1])
	{
		translate([side * distance / 2, 0, 0])
			hotend_nut_holder();
	}
}

module set_of_hotend_nut_holders_holes(distance){
	for(side = [-1,1])
	{
		translate([side * distance / 2, 0, 0])
			hotend_nut_holder_holes();
	}
}

module hotend_nut_holder()
	{
		height = 8;
		rotate(90)
		translate([0, 0, 4])
...
module hotend_nut_holder_holes()
	{
	height = 8;
...
module 40mmFan() {
 ...
 rotate([0,90,0])
 union(){
	 cube([40,40,11], center=true);
	 for (half = [-1,1])
		for (side = [-1,1])
			translate([32.5/2*side,32.5/2*half,15])
	...
	difference(){
		...
		union()
		{
			// Servo Hole
			translate([-8,-58,2])
				cube([servo_width,servo_depth+7,servo_height+4]); // main body cavity
			translate([12,-48,2])
				cube([10,10,11]);  // opening for wires
			translate([10,-44,2])
				cube([10,10,11]);  // trim off the side wall				
			
            // Holes for servo screws
			for (side=[0,1])
			translate([-10.5 + (side * 28.5),-40,8.5])
			rotate([90,0,0])
			cylinder(30,1.5,1.5);			
